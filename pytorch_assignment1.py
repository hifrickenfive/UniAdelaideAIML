# References to understand ML workflow
...
class NN_CNN(nn.Module):
    def __init__(self):
        ...
        self.conv_layers = nn.Sequential(
            ...
            nn.Conv2d(10, 40, kernel_size=5),
            nn.Dropout(),
            nn.MaxPool2d(2),
            nn.ReLU(),

            nn.Conv2d(40, 200, kernel_size=3),
            nn.Dropout(),
            nn.MaxPool2d(2),
            nn.ReLU(),
        )
        self.fc_layers = nn.Sequential(
            nn.Linear(200, 50), #2nd layer creats 20x4x4 outputs. 3rd layer creates 40 outputs x 1 x 1 as f(kernel size, padding, output of layer 2)
            nn.ReLU(),
            nn.Dropout(),
            nn.Linear(50, 10),
            nn.Softmax(dim=1)
...
for t in range(epochs):
    ...
    test(validation_loader, model, loss_function)
print("Done!")